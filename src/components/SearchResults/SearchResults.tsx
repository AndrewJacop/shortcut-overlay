...
const SearchResults: FC<SearchResultsProps> = ({
	...
	// Group results by sheet+page+section for visual sectioning
	type GroupKey = string;
	const groups = new Map<GroupKey, SearchResult[]>();
	for (const r of results) {
		const key: GroupKey = `${r.sheetId}::${r.pageIndex}::${r.sectionIndex ?? -1}`;
				...
				return (
					<div key={key}>
						...
						{groupResults.map((r) => {
							...
							return (
								<div
									key={`${r.sheetId}-${r.pageIndex}-${r.sectionIndex ?? ""}-${r.shortcutIndex}`}
